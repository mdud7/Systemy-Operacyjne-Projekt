...
int main() {
	...
	if (flag_check_msg) {
            ManagerOrderMsg msg;
            while (msgrcv(msgid, &msg, sizeof(ManagerOrderMsg), 1, IPC_NOWAIT) != -1) {
                sem_lock(semid);
                int reserved_cnt = 0;
                for (int i = 0; i < shm->table_count && reserved_cnt < msg.count; i++) {
                    if (shm->tables[i].current_count == 0 && shm->tables[i].is_reserved == 0) {
	...
	if (flag_triple_tables) {
		sem_lock(semid);

		if (shm->x3_doubled) {
			log_acti
on("blad: stoliki juz zwiekszone");
		} else {
				...
				char buf[128];
				sprintf(buf, "Potrojono stoliki 3-osobowe. Było: %d, Dodano: %d (Suma: %d).", 
              	        		count_x3, added, count_x3 + added);
                		log_action(buf);
            		}

			sem_unlock(semid);
        	    	flag_triple_tables = 0;
		}
